...
export const fetchFavor = (guestId) => {
    return async dispatch => {
        try {
            ...
            const response = await axios.get(`https://191.44.38.17:444/favor/${guestId}`)
...
export const sendFavor = (doorId, guestId) => {
    return async dispatch => {
        try {
            const response = await axios.post(`https://191.44.38.17:444/favor`, {doorId, guestId});
            if (response.data !== '') {
...
export const deleteFavor = (doorId, guestId) => {
    return async dispatch => {
        try {
            const response = await axios.delete(`https://191.44.38.17:444/favor`, {data: {doorId, guestId}})
